...
export const ClassPDFAttachments: React.FC<EvaluationAttachmentProps> = ({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload dans le storage en premier
      const { error: storageError } = await supabase.storage
        .from('evaluation-attachments')
        .upload(filePath, file, {
          upsert: false, // Pas d'upsert pour éviter le remplacement
      ...
      // Insertion dans la table après un upload réussi
      ...
      // Si l'upload ou l'insertion échoue, nettoyer le storage si nécessaire
  ...
  const handleDeleteFile = async (attachment: AttachmentRecord) => {
    const confirmed = window.confirm(
      'Tem certeza que deseja remover este anexo? Esta ação não pode être desfeita.'
    );
    if (!confirmed) return;
    try {
      setLoading(true);
