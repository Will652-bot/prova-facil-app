...
interface EvaluationPDFAttachmentsProps {
...
export const EvaluationPDFAttachments: React.FC<EvaluationPDFAttachmentsProps> = ({
  evaluationTitleId,
  evaluationTitle
}) => {
  const { user } = useAuth();
  ...
  const fetchAttachment = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('evaluation_title_id', evaluationTitleId) // <-- Cible la bonne colonne
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `${user.id}/${evaluationTitleId}/${filename}`; // S'assure d'avoir un chemin unique
      ...
      const { error: storageError } = await supabase.storage
        .from('evaluation-attachments') // <-- Utilise le bon bucket
      ...
      const newAttachmentData = {
        evaluation_title_id: evaluationTitleId, // <-- Utilise le bon ID de clé étrangère
        teacher_id: user.id,
        file_path: filePath
      };
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase
        .from('evaluation_attachments')
        .delete()
        .eq('evaluation_title_id', evaluationTitleId) // <-- Utilise la bonne colonne pour la suppression
        .eq('teacher_id', user?.id);
      ...
      {!attachment && !uploading && (
        <div className="text-center py-2">
          ...
          <p className="text-sm text-gray-500">
            Nenhum arquivo PDF anexado para este título de avaliação.
          </p>
          {!isPro && (
